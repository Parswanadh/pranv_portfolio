...
async function testPage(browser, pageInfo) {
  ...
  // Navigate to page
  try {
    // Try with commit instead of domcontentloaded to avoid timing issues
    ...
    await page.waitForLoadState('domcontentloaded', { timeout: 10000 }).catch(() => {
      errors.push({
        text: 'Page did not reach domcontentloaded state within timeout',
        location: 'navigation'
      });
    });

    // Additional wait for any delayed console messages
...
async function main() {
  ...
  for (const pageInfo of allPages) {
    ...
    try {
      ...
      console.log(`  Total Console Messages: ${result.allConsoleMessages.length}`);
    } catch (error) {
  ...
  // All console messages by page
  markdown += `## All Console Messages by Page\n\n`;
  for (const result of results) {
    if (result.allConsoleMessages && result.allConsoleMessages.length > 0) {
      markdown += `### ${result.page} (${result.url})\n\n`;
      result.allConsoleMessages.forEach(msg => {
        markdown += `- [${msg.type.toUpperCase()}] ${msg.text}\n`;
        if (msg.location !== 'unknown') {
          markdown += `  at ${msg.location}\n`;
        }
      });
      markdown += `\n`;
    }
